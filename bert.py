...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    scores = scores/math.sqrt(self.all_head_size)
    #masking attention heads
    scores += attention_mask
    #normalizing and dropout
    probabilities = nn.Softmax(dim=-1)(scores)
    probabilities = self.dropout(probabilities)
    #reshaping and applying
    context = torch.matmul(probabilities, value)
    context = context.transpose(1, 2)
    context = context.reshape(context.shape[0], context.shape[1], self.all_head_size)
    return context
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    hidden_states = self.add_norm(hidden_states, attention,
                                  self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    placehold = self.interm_dense(hidden_states)
    placehold = self.interm_af(placehold)

    output = self.add_norm(hidden_states, placehold, self.out_dense, self.out_dropout, self.out_layer_norm)
    return output
    ####raise NotImplementedError
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    #  Get word embedding from self.worOd_embedding into input_embeds.
    ...
    ### TODO
    tk_types = torch.zeros(input_shape, dtype=torch.long, device=input_ids.device)
    tk_embeddings = self.tk_type_embedding(tk_types)
    #raise NotImplementedError
    ...
    ### TODO
    placehold = inputs_embeds+ pos_embeds+tk_type_embeds
    placehold = self.embed_layer_norm(placehold)
    placehold = self.embed_dropout(placehold)
    return placehold
